...
async fn can_get_proof() {
    ...
    api.anvil_set_storage_at(acc.to_alloy(), key.to_alloy(), B256::from_uint(&value).to_alloy())
        .await
        .unwrap();

    let proof: EIP1186AccountProofResponse =
        api.get_proof(acc.to_alloy(), vec![B256::from_uint(&key).to_alloy()], None).await.unwrap();
    ...
    let root: B256 = api.state_root().await.unwrap().to_ethers();
    let acc_proof: Vec<Vec<u8>> = proof
        .account_proof
        .into_iter()
        .map(|node| alloy_rlp::decode::<Vec<u8>>(&node).unwrap())
        .collect();
    ...
    let storage_proof: Vec<Vec<u8>> =
        proof.proof.into_iter().map(|node| alloy_rlp::decode::<Vec<u8>>(&node).unwrap()).collect();
